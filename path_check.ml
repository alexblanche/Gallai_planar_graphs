...
(** Check that a color col is a path **)
    ...
    then false
      ...
      if (count (fun i -> save_and_check.(i) = 1) vcol) <> 2
        then false
          ...
          (let uf = init_union_find vcol in
          List.iter (fun i -> List.iter (fun e -> union uf i e.edge_end) (neigh_col cg col i)) vcol;
          check_one_component uf)
;;
  



(** Check that the coloring of the colored graph cg is a path-decomposition **)
